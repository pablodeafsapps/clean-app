...
class RegisterUserUcTest : KoinTest {
    ...
    fun setUp() {
        ...
        startKoin {
            modules(listOf(
                domainLayerModule,
                module { single(named(name = DomainlayerContract.Datalayer.AUTHENTICATION_REPOSITORY_TAG)) { mockRepository } }
            ))
        }
    }
    ...
    @Test
    fun `check that if user login data are not null or empty, registerUer is invoked`() = runBlockingTest {
        // given
        val rightParams = UserLoginBo(email = "example@cleanapp.es", password = "password")
        // when
