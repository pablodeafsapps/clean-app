...
import es.plexus.android.plexuschuck.domainlayer.DomainlayerContract.Datalayer.Companion.JOKES_DATA_REPOSITORY_TAG
...
val dataLayerModule = module(override = true) {
    factory<DataLayerContract.AuthenticationDataSource>(name = AUTHENTICATION_DATA_SOURCE_TAG) { FirebaseDataSourceImpl() }
    factory<DataLayerContract.JokesDataSource>(name = JOKES_DATA_SOURCE_TAG) { IcndbDataSourceImpl() }

    single<DomainlayerContract.Datalayer.FirebaseRepository<List<String>, Boolean>>(name = AUTHENTICATION_REPOSITORY_TAG) { Repository }
    single<DomainlayerContract.Datalayer.IcndbRepository<List<String>?, List<JokeBo>>>(name = JOKES_DATA_REPOSITORY_TAG) { Repository }
}