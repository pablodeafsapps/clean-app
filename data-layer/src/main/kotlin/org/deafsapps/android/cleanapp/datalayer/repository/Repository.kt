...
object Repository : DomainlayerContract.Datalayer.Repository<String>, KoinComponent {
    ...
    override fun loginUser(params: List<String>): Either<FailureBo, Boolean> {
        return firebaseDataSource.requestLogin(email = params[0], password = params[1])?.let {
            if (it) Either.Right(it) else Either.Left(
                org.deafsapps.android.cleanapp.datalayer.domain.dtoToBoFailure(
                    FailureDto.FirebaseLoginError
                )
            )
        } ?: run {
            Either.Left(org.deafsapps.android.cleanapp.datalayer.domain.dtoToBoFailure(FailureDto.Unknown))
        }
    }
    ...
    override fun registerUser(params: List<String>): Either<FailureBo, Boolean> {
        return firebaseDataSource.requestRegister(email = params[0], password = params[1])?.let {
            if (it) Either.Right(it) else Either.Left(
                org.deafsapps.android.cleanapp.datalayer.domain.dtoToBoFailure(
                    FailureDto.FirebaseRegisterError
                )
            )
        } ?: run {
            Either.Left(org.deafsapps.android.cleanapp.datalayer.domain.dtoToBoFailure(FailureDto.Unknown))
